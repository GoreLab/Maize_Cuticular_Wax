...
initial.outliers <- function(transfpheno, curr.trait, random, fixed, nbvariables, BLUx) {
    originalpheno <- transfpheno ## keep an intact copy
    nb.outliers <- 1
    hold <- vector()
    significance <- vector()
    iteration <- 0
    ...
    Mycol <- rep('black', dim(transfpheno)[1] ) # needs to be defined here in case there are no outliers
    #
    ##if (nb.outliers > 0) {  # just to debug
    while (nb.outliers > 0) {
        # Bonferroni corrected cutoff
        nbind <- eval(parse(text = paste('length(transfpheno[which(!is.na(transfpheno$',curr.trait,')),]$',curr.trait,')', sep='')))
        ...
        if (length(fixed) == 0) {
            ...
            fullmodel <- paste( "asreml(fixed = ", curr.trait, " ~ 1 ",
                               ...
                               ", na.method.X = 'include', maxiter=100, data = transfpheno, aom=TRUE)" )
        }
        if (length(fixed) > 0) {
        ...
        if (nb.outliers > 0) {
            ...
            ### Update transfpheno, putting NA where we detected outliers
            #
            eval(parse(text= paste('transfpheno[which(abs(studentizedRes) > ', threshold, '),]$', curr.trait, '<- NA', sep='')))
            hold <- c(hold, which(abs(studentizedRes) > threshold))
            #
            ### Write to pdf
            Mycol[ which(abs(studentizedRes) > threshold) ] <- 'red'
            plot(studentizedRes, col=Mycol, main=paste('Iteration ', iteration, sep=''))
            ...
            plot(studentizedRes, col=Mycol, ylim=c(-threshold, threshold), main='No outlier')
            abline(h=threshold, col='red')
            abline(h=-threshold, col='red')
            dev.off()
        }
    ...
    tmp.trait <- eval(parse(text = paste('transfpheno$', curr.trait, sep='')))
    return(tmp.trait)
}
