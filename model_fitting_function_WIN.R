...
## Note: Meng's editing (02112020):
## 1. force the baseline model into the best model to ensure the model has genotype and gxe
## 2. add CHECK_99 effect to BLUPs to avoid negative BLUP values (good for transformation if needed)

library(asreml)

fitmodel <- function(cleanedpheno, curr.trait, random, fixed, nestedterms, PREDICTION, BLUx,baselinemodel) { # Meng added "baselinemodel"
    ...
    if (length(fixed) == 0) {
        ...
        fullmodel <- paste( "asreml(fixed = ", curr.trait, " ~ 1 ",
                           ...
                           ", na.method.X = 'omit', maxiter=100, data = cleanedpheno)" )
    }
    if (length(fixed) > 0) {
    ...
    ### Meng's edits: force to include terms in the baseline model to "randomInBestModel"
    randomInBestModel<-c(randomInBestModel,baselinemodel)
    ## End of Meng's edits
    ...
    if (length(fixed) != 0) {
        ...
        fixed.effects <- wald.asreml(BM.asr, denDF = 'numeric', ssType = 'conditional', pworkspace=320e6)
    ...
    dev.off()
    #
    predict(BM.asr, classify=PREDICTION, data=cleanedpheno, pworkspace=320e6)-> blux
    #
    ...
    if (BLUx == 'BLUES') {
        ...
        return(list(model = summary(BM.asr), best.model = BM.asr, blues = blux$predictions$pvals, anova = fixed.effect.anova) )
    }
    if (BLUx == 'BLUPS') {
        # Same as BLUEs, except that we need to add the grand mean to the estimates to get the "real" BLUPs
        # Extract the model coefficients
        random.ef <- data.frame(BM.asr$coefficients$random)
        fixed.ef <- BM.asr$coefficients$fixed
        # Get grand mean to add back to BLUPs
        intercept <- fixed.ef[grep('Intercept', names(fixed.ef))]
        check99 <- fixed.ef[grep('CHECK_99', names(fixed.ef))] ## added by Meng to account for check effect and avoid negative values in BLUPs
        #BLUPS <- random.ef + intercept
        BLUPS <- random.ef + intercept + check99 ## added by Meng to account for check effect and avoid negative values in BLUPs
        ...
        BLUPS <- all.na
        return(list(model = summary(BM.asr), best.model = BM.asr, blups = BLUPS, anova = fixed.effect.anova) )
    }
}
