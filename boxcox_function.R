...
boxcox.fitting <- function(pheno, curr.trait, baselinemodel) {
...
# if it is, then both the untransformed and transformed values are output.
    ...
    bxc <- paste( "boxcox(", curr.trait, "~", paste(baselinemodel,collapse=" + "), ",data = pheno, lambda = seq(-2, 2, .05))")
    pdf('lambda_values.pdf')
    ...
    # If not, run the boxcox for the next two adjacent convenient values and take the one that maximizes the likelihood
        ...
        bxc1 <- paste( "boxcox(", curr.trait, "~", paste(baselinemodel,collapse=" + "), ", plotit = FALSE, data = pheno, lambda = seq(", paste(convenient.lambdas.to.test[1],",",convenient.lambdas.to.test[2]), ", length=2))")
