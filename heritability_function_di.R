...
pin <- function (fullmodel, transform, augmented=F) {
    ...
    ### JC: End modification
    ################################################################################
    names(pframe)[gammaterms.with.G] <- VCterms[1]
...
calc.herit <- function (fullmodel, curr.trait, all.harm.mean, augmented=F){  # all_harm.mean is a vector with all the harmonic means calculated above
    ...
    colnames.to.keep <- c(colnames.to.keep, VCterms[1])
    # residual term
    ...
    Terms.with.G[which(colnames(VC) == VCterms[1])] <- 1
    ...
    transformModel <- paste(VCterms[1], ' / (' , collapsedterms , ')')
    ...
    transformModel <- paste(VCterms[1], ' / (' , allcollapsedterms , ')')
    ################################################################################
    ### JC: Modification to pass the argument for 'augmented' parameter
    ################################################################################
    ...
    return(list(h2plot=Heritability.plot.basis, h2family=Heritability.family.basis, h2plot.pin=h2.pin.plot, seplot.pin=se.pin.plot, h2family.pin=h2.pin.family, sefamily.pin=se.pin.family))
}
